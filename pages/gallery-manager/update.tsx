...
class GalleryUpdatePage extends PureComponent<IProps, IStates> {
  ...
  async onFinish(data: IGalleryCreate) {
    try {
      ...
      Router.push('/gallery-manager/listing');
    }
  }
  ...
  render() {
    const { ui } = this.props;
    const { gallery, submitting, loading, photos } = this.state;
    return (
      <Layout>
        <Head>
          <title> {ui && ui.siteName} | Update Gallery </title>
        </Head>
        <Content>
          <div className="main-container">
            {/* <BreadcrumbComponent
              breadcrumbs={[
                { title: 'Gallery Manager', href: '/gallery-manager/listing'},
                {
                  title:
